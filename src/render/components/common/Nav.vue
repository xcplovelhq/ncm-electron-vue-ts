    ...
    <div class="g-nav">
        <div class="g-avata" @click.stop="handleLogin">
            <div class="m-avata" @click.stop="handleDetails">
                <el-image style="width: 100px; height: 100px" :src="user.avatarUrl" fit="fill"></el-image>
            </div>
            <div class="m-name">
                {{ user.nickname || '未登录' }}
                <i class="iconfont">&#xe618;</i>
        ...
        <div class="m-nav-main">
            ...
            <div class="m-nav-my">
                <router-link class="m-nav-item" v-for="item in myNav" :key="item.key" :to="item.link" :class="{ active: item.isActive }">
                    <i class="iconfont" v-html="item.icon"></i>
                    {{ item.name }}
                </router-link>
            </div>
            ...
            <div class="m-nav-collapse" v-if="songSheetList && songSheetList.length > 0" @click="isShowCollapse = !isShowCollapse">
                <i class="iconfont">&#xe618;</i>
                收藏的歌单
            </div>
...
export default defineComponent({
    setup() {
        // console.log(11111);
        const isShowFound = ref(true);
        const isShowCollapse = ref(true);
        const onlineNav = reactive({
            onlineNav: [
                {
                    name: '发现音乐',
                    icon: '&#xe700',
        ...
        const user = computed(state => {
            return state.User.userInfo;
        });
        const songSheetList = computed(state => {
