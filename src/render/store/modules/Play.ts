...
const actions = {
    async getSongUrl({ commit }, payload) {
        const { data } = await api.musicData.getSongUrl(payload);
        if (data.code === 200) {
    ...
    async getLyric({ commit }, payload) {
        const { data } = await api.musicData.getLyric(payload);
        if (data.code === 200) {
            if (payload.type === 'FM') {
                storage('fmLyric', data);
    ...
    async getSongDetails({ dispatch, commit, state }, payload) {
        ...
        const playIdx = playList.findIndex(function(obj) {
            return obj ? obj.id === playInfo.id : -1;
        });
        const rowIdx = playList.findIndex(function(obj) {
        ...
        const { data } = await api.musicData.getSongDetails(payload);
        if (data.code === 200) {
            if (playIdx >= 0) {
